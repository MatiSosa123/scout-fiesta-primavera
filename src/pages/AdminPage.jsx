  ...
  return (
    <div className="min-h-screen bg-slate-100 p-4 md:p-6">
      <div className="max-w-5xl mx-auto space-y-4 md:space-y-6">
        ...
        <div className="bg-white p-4 md:p-6 rounded-lg shadow-md flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="text-center md:text-left">
            <h1 className="text-2xl md:text-3xl font-bold text-slate-800">🌸 Panel de Administración</h1>
            <p className="text-slate-500 text-sm md:text-base">Control de Entradas - Manada y Tropa</p>
          </div>
          <div className="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
            <div className="text-center md:text-right w-full md:w-auto bg-green-50 p-3 rounded-lg border border-green-200">
              <p className="text-xs text-green-700 uppercase font-bold">Recaudación Total</p>
              <p className="text-2xl md:text-3xl font-bold text-green-600">${calcularTotal().toLocaleString('es-AR')}</p>
            </div>
            <button 
              onClick={() => signOut(auth)} 
              className="flex items-center justify-center gap-2 bg-red-100 hover:bg-red-200 text-red-700 font-semibold px-4 py-2 rounded-lg transition-colors w-full md:w-auto"
            >
              <LogOut size={18} />
              Salir
            </button>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
          <div className="bg-white p-3 md:p-4 rounded-lg shadow-sm border-l-4 border-blue-400 flex items-center justify-between">
            <div><p className="text-[10px] md:text-xs text-slate-500 font-semibold uppercase">Scouts</p><p className="text-xl md:text-2xl font-bold text-slate-700">{stats.scouts}</p></div>
            <Users className="text-blue-400" size={24} />
          </div>
          <div className="bg-white p-3 md:p-4 rounded-lg shadow-sm border-l-4 border-purple-400 flex items-center justify-between">
            <div><p className="text-[10px] md:text-xs text-slate-500 font-semibold uppercase">Invitados</p><p className="text-xl md:text-2xl font-bold text-slate-700">{stats.invitados}</p></div>
            <UserCheck className="text-purple-400" size={24} />
          </div>
          <div className="bg-white p-3 md:p-4 rounded-lg shadow-sm border-l-4 border-pink-400 flex items-center justify-between">
            <div><p className="text-[10px] md:text-xs text-slate-500 font-semibold uppercase">Padres</p><p className="text-xl md:text-2xl font-bold text-slate-700">{stats.padres}</p></div>
        ...
        <div className="bg-white p-4 md:p-6 rounded-lg shadow-md">
          <h2 className="text-lg md:text-xl font-semibold mb-4 text-slate-700 flex items-center gap-2"><UserPlus size={20} /> Agregar Asistente</h2>
          <form onSubmit={agregarAsistente} className="grid grid-cols-1 md:grid-cols-5 gap-3 md:gap-4">
            <input type="text" placeholder="Nombre" required className="border p-2 rounded focus:outline-blue-500 text-sm" value={form.nombre} onChange={e => setForm({...form, nombre: e.target.value})} />
            <input type="text" placeholder="Apellido" required className="border p-2 rounded focus:outline-blue-500 text-sm" value={form.apellido} onChange={e => setForm({...form, apellido: e.target.value})} />
            <input type="number" placeholder="DNI" required className="border p-2 rounded focus:outline-blue-500 text-sm" value={form.dni} onChange={e => setForm({...form, dni: e.target.value})} />
            <select className="border p-2 rounded focus:outline-blue-500 text-sm" value={form.tipo} onChange={e => setForm({...form, tipo: e.target.value})}>
            ...
            <select className="border p-2 rounded focus:outline-blue-500 text-sm" value={form.pago} onChange={e => setForm({...form, pago: e.target.value})} disabled={form.tipo === 'Padre'}>
              <option value="Preventa">Pagó Preventa ($6.000)</option>
              <option value="Puerta">Pagó en Puerta ($7.000)</option>
              <option value="Debe">Debe</option>
            </select>
            <button type="submit" className="md:col-span-5 bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded transition-colors text-sm">Agregar a la lista</button>
          </form>
        </div>
        ...
        <div className="bg-white p-4 md:p-6 rounded-lg shadow-md">
          <div className="flex flex-col md:flex-row gap-3 md:gap-4 mb-4">
            ...
            <div className="flex gap-2 w-full md:w-auto">
              <select value={orden} onChange={e => setOrden(e.target.value)} className="border p-2 rounded bg-slate-50 text-xs md:text-sm font-medium focus:outline-blue-500 flex-1 md:flex-none">
                <option value="default">Orden de carga</option>
                <option value="nombre">Nombre (A-Z)</option>
                <option value="apellido">Apellido (A-Z)</option>
              </select>
              <button onClick={exportarACSV} className="flex items-center justify-center gap-2 bg-green-600 hover:bg-green-700 text-white font-semibold py-2 px-3 md:px-4 rounded transition-colors text-xs md:text-sm flex-1 md:flex-none">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b-2 border-slate-200 text-slate-600 text-xs md:text-sm">
                  <th className="p-2 md:p-3">Asistió</th>
                  <th className="p-2 md:p-3">Nombre</th>
                  {/* Ocultamos DNI y Tipo en celulares */}
                ...
                {asistentesFiltradosYOrdenados.length === 0 ? (
                  ...
                  asistentesFiltradosYOrdenados.map(a => (
                    <tr key={a.id} className="border-b border-slate-100 hover:bg-slate-50 text-sm">
                      ...
                      <td className="p-2 md:p-3 font-medium text-slate-800">{a.nombre} {a.apellido}</td>
                      {/* Ocultamos DNI y Tipo en celulares */}
                      <td className="p-2 md:p-3 text-slate-600 hidden md:table-cell">{a.dni}</td>
                      <td className="p-2 md:p-3 hidden md:table-cell">
                        <span className={`px-2 py-1 rounded text-[10px] font-semibold ${a.tipo === 'Padre' ? 'bg-purple-100 text-purple-700' : 'bg-blue-100 text-blue-700'}`}>{a.tipo}</span>
                      </td>
                      <td className="p-2 md:p-3">
